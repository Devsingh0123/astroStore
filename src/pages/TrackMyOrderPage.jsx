              ...
              <div className="py-2">
                ...
                <div className="hidden sm:grid grid-cols-4 relative">
                  ...
                  {STEPS.map((step, idx) => {
                    ...
                    if (
                      step.key === "confirmed" &&
                      order.timestamps?.confirmed_at
                    )
                      timeStamp = order.timestamps.confirmed_at;
                    if (step.key === "packed" && order.timestamps?.packed_at)
                      timeStamp = order.timestamps.packed_at;
                    if (step.key === "shipped" && order.timestamps?.shipped_at)
                      timeStamp = order.timestamps.shipped_at;
                    if (
                      step.key === "delivered" &&
                      order.timestamps?.delivered_at
                    )
                      timeStamp = order.timestamps.delivered_at;
                    ...
                    return (
                      ...
                      >
                        ...
                        <p
                          className={`text-xs font-semibold mt-2.5 ${isCompleted ? "text-gray-900" : "text-gray-400"}`}
                        >
                          {step.label}
                        </p>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="pt-0.5">
                          <p
                            className={`text-xs font-semibold ${isCompleted ? "text-gray-900" : "text-gray-400"}`}
                          >
                            {step.label}
                          </p>
                          {timeStamp && (
            ...
            <div className="pt-4 border-t border-gray-100 flex flex-wrap items-center justify-between text-xs text-gray-500 gap-2">
              <div>
                ...
                <span className="font-semibold text-gray-700 uppercase">
                  {order.payment?.mode || order.payment?.method || "Online"}
                </span>
              </div>
              <div>
                Total:{" "}
                <span className="font-bold text-gray-900">
                  ₹
                  {parseFloat(
                    order.pricing?.total_amount || order.total || 0,
                  ).toFixed(2)}
                </span>
              </div>
            </div>
