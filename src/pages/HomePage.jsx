...
const HomePage = () => {
  ...
  // State for navbar height and visibility
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Detect if navbar should be visible (scrolling up or at top)
      if (currentScrollY < lastScrollY.current - threshold) {
        ...
        setIsNavbarVisible(false); // scrolling down past navbar
      }
  ...
  // Dynamic top for HomeNav: if navbar is hidden, stick to 0, else stick below navbar
  const homeNavTop = isNavbarVisible ? navbarHeight : 0;
  // Sidebar top: always below HomeNav (with gap)
  const sidebarTop = homeNavTop + 60 + 8; // 60 is approx HomeNav height
    ...
    <div className="px-2 sm:px-4">
      <div className="flex gap-3 sm:gap-4 md:gap-6">
        ...
        <div className="flex-1 min-w-0">
          {/* HomeNav - Sticky with dynamic top */}
          <div
          ...
          {/* Sidebar aur Content side-by-side */}
          <div className="flex gap-3 sm:gap-4 md:gap-6 mt-4">
            ...
            >
              <div
                className="bg-white rounded-xl border border-gray-200 p-3 sm:p-4 w-full overflow-y-auto scrollbar-hide"
